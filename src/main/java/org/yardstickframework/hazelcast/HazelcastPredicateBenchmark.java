 ...
 */
public class HazelcastPredicateBenchmark extends HazelcastAbstractMapBenchmark {
    ...
    /** */
    public HazelcastPredicateBenchmark() {
        super("query");
    }
    ...
    private Collection<Person> executeQuery(double minSalary, double maxSalary) throws Exception {
        ...
        Predicate p1 = entryObject.get("salary").greaterEqual(minSalary);
        Predicate p2 = entryObject.get("salary").lessEqual(maxSalary).and(p1);

        return (Collection<Person>)(Collection<?>)map.values(p2);
    }
}
